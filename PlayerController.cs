...
{
    public InvincibilityController m_InvincibilityController; //プレイヤーの操作を受け付けなくさせる(無敵時間用)
    ...
    void Start()
    {
        ...
        m_InvincibilityController = GameObject.Find("GameManager").GetComponent<InvincibilityController>();
    ...
    public void Damage()
    {
        if (m_Invincibility == false)
        {
            ...
            else if (m_HP > 0)
            {
                ...
                m_InvincibilityController.CallInoperable(1.5f); //指定した秒数このスクリプトを無効化する
